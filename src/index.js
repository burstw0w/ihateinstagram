...
async function handleUrl(msg) {
  ...
  if (check) {
    ...
    if (check[0].includes("instagram")) {
      ...
      console.log(match[0]);
      let instagramPattern =
        /\bhttps?:\/\/(www\.)?instagram\.com\/[\w/]+\/\S*/gi;
        ...
        if (match[0] === "nema") {
          ...
          if (!isNaN(match[0])) {
            ...
            if (fileList.length === 0) {
              await msg.reply({
                content: "Нисам успео извући ништа из овог поста.",
              });
            } else {
              ...
              if (obrisan === 0) {
                ...
                await msg.reply({
                  content:
                    `${msg.member.user.tag} uploads, али неке ствари нису ту јер су превелике` +
                    "<" +
                    check[0] +
                    ">" +
                    ":",
      ...
      await msg
        .reply({
          content:
            msg.member.user.tag + ` uploads ` + "<" + check[0] + ">" + ":",
          files: [fnamett],
        })
        .catch((err) => {
          console.log("Error during Export File " + err);
...
async function pintVideo(redditURL) {
  ...
  try {
    ...
    const errorMessage = await page.$eval("div.text-xl", (el) =>
      el.textContent.trim()
    );
    if (
      errorMessage.includes("Sorry, we could not find any video on this Pin.")
    ) {
